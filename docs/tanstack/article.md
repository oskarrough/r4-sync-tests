...
Now, usually what happens is you fetch data from an API, say a list of music __tracks__, you pass it to a component (template) which renders it to HTML. That's it.
...
The cache is controlled by primarily two things: 
	1. Time-to-live (TTL), for example one minute and
	2. Garbage collection (gc), for example 24 hours.
...
The safest would be to 
	1) update data via API (maybe 400ms)
	2) request new data (maybe 200ms)
