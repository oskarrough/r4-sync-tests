...
onUpdate: async ({transaction}) => {
	...
	if (mutation.metadata?.intent === 'complete') {
		await Promise.all(transaction.mutations.map((mutation) => api.todos.complete(mutation.original.id)))
	} else {
		await Promise.all(transaction.mutations.map((mutation) => api.todos.update(mutation.original.id, mutation.changes)))
	}
}
```
...
todoCollection.insert({id: '1', text: 'Custom item', completed: false}, {metadata: {source: 'import'}})

// Insert without optimistic updates
todoCollection.insert({id: '1', text: 'Server-validated item', completed: false}, {optimistic: false})
```
...
todoCollection.update(todo.id, {metadata: {reason: 'user update'}}, (draft) => {
	draft.text = 'Updated text'
})

// Update without optimistic updates
todoCollection.update(todo.id, {optimistic: false}, (draft) => {
...
```typescript
type OperationHandler = (params: {transaction: Transaction; collection: Collection}) => Promise<any> | any
```
...
const addTodoTx = createTransaction({
	...
	mutationFn: async ({transaction}) => {
		...
		await Promise.all(transaction.mutations.map((mutation) => api.saveTodo(mutation.modified)))
	}
})
...
const tx = createTransaction({
	...
	mutationFn: async ({transaction}) => {
		...
		const draftData = transaction.mutations.filter((m) => m.collection === formDraft).map((m) => m.modified)
...
const tx = createTransaction({
	mutationFn: async ({transaction}) => {
		...
		await Promise.all(
			transaction.mutations.filter((m) => m.collection === userProfile).map((m) => api.profile.update(m.modified))
		)
...
function VolumeSlider() {
	...
	return <input type="range" min={0} max={100} onChange={(e) => handleVolumeChange(Number(e.target.value))} />
}
```
...
const handleUpdateTodo = async (todoId, changes) => {
	const tx = todoCollection.update(todoId, {optimistic: false}, (draft) => Object.assign(draft, changes))
...
const TodoItem = ({todo, isPersisted}: {todo: Todo; isPersisted: boolean}) => {
	return (
		<div>
			{todo.text}
			<button onClick={() => todoCollection.delete(todo.id)} disabled={!isPersisted}>
				Delete
			</button>
		</div>
	)
